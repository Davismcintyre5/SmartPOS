...
export default function POS() {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      if (bridge) {
        const cached = await bridge.db.getProducts();
        if (cached.ok && cached.products.length > 0) {
      ...
      const res = await productApi.list({ limit: 200, active: true });
  ...
  const handlePay = async (
    ...
    try {
      ...
      let receiptSale: ReceiptSale;

      if (bridge) {
        const statusRes = await bridge.sync.getStatus();
        const online = statusRes.ok && statusRes.status?.online === true;

        if (!online) {
          const normalizedItems = items.map((i) => ({
            productId: i._id,
            name: i.name,
            sku: (i as Product).sku || null,
            qty: i.quantity,
            price: whole(i.price),
            subtotal: whole(i.price) * i.quantity,
          }));

          const enq = await bridge.sync.enqueueSale({
          ...
          if (!enq.ok) throw new Error(enq.error || 'Failed to queue sale');

          receiptSale = {
            saleNumber: `OFF-${String(enq.localId || '').slice(0, 8)}`,
            items: items.map((i) => ({
              name: i.name,
              qty: i.quantity,
              price: i.price,
          ...
          const { data } = await api.post<{ data: Sale }>('/client/sales', payload);
          receiptSale = buildReceiptSale(data.data, method, paid, change);
        }
      } else {
        const { data } = await api.post<{ data: Sale }>('/client/sales', payload);
        receiptSale = buildReceiptSale(data.data, method, paid, change);
